...
describe.skip('closestEvent', () => {
  it('should return the name of the closest event', () => {
    ...
    expect(closestEvent(events, new Date(1990, 0, 1))).toBe('Fall of the Berlin Wall');
    expect(closestEvent(events, new Date(1995, 9, 11))).toBe('Fall of the Berlin Wall');
    expect(closestEvent(events, new Date(1995, 9, 12))).toBe('September 11 Attacks');
    expect(closestEvent(events, new Date(2005, 0, 1))).toBe('Indian Ocean Earthquake and Tsunami Disaster');
    expect(closestEvent(events, new Date(2006, 5, 16))).toBe('Hurricane Katrina');
    expect(closestEvent(events, new Date(2007, 4, 12))).toBe('Hudson River Plane Crash');
